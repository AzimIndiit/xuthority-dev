...
import { usePublicProfileBySlug, useUserReviewsBySlug, useUserProfileStats } from '@/hooks/useAuth';
...
const PublicProfileBySlugPage: React.FC = () => {
  ...
  // Fetch user reviews by slug
  const { 
    data: reviewsData, 
    isLoading: reviewsLoading, 
    error: reviewsError 
  } = useUserReviewsBySlug(slug || '', {
    page,
    limit: 10,
    sortBy,
    sortOrder
            ...
            <Card className="bg-white border border-gray-200 shadow-sm">
              <CardContent className="p-6">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-600 text-sm">{publicProfile?.email}</p>
                  <p className="text-gray-500 text-xs mt-1">@{publicProfile?.slug}</p>
                  <Button 
                    variant="outline"
                    size="sm"
                    onClick={handleFollowToggle}
                    className="mt-3 w-full"
                    disabled={toggleFollowMutation.isPending || !publicProfile?._id}
                  >
              ...
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold text-gray-900">
                    {publicProfile?.firstName} Reviews
                  </h2>
